...
class Ball(Agent):
    def __init__(
        ...
        self.vy = vy
        if color is None:
            self.color = choice(['black', 'blue', 'gray', 'brown'])
        else:
            self.color = color
...
class Target(Agent):
    def __init__(
        ...
        x = self.x = rnd(400, 750) if x is None else x
        y = self.y = rnd(150, 500) if y is None else y
        r = self.r = rnd(2, 50) if r is None else r
        # color = self.color = 'red' if color is None else color
        color = self.color = choice(['purple', 'yellow', 'maroon', 'gold', 'green'])
